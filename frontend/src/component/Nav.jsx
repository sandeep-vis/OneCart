...
function Nav() {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Clear user data in context + localStorage
      setUserData(null);
    ...
    <div className="w-[100vw] h-[70px] bg-[#ecfafaec] z-10 fixed top-0 flex items-center justify-between px-[30px] shadow-md shadow-black">
      ...
      <div className="w-[50%] lg:w-[40%] hidden md:flex">
        <ul className="flex items-center justify-center gap-[19px] text-white">
          ...
          <li className="nav-item" onClick={() => navigate("/collection")}>COLLECTIONS</li>
      ...
      {/* Right Icons */}
      <div className="w-[30%] flex items-center justify-end gap-[20px]">
        {!showSearch && <IoSearchCircleOutline className="icon" onClick={() => { setShowSearch(true); navigate("/collection"); }} />}
        {showSearch && <IoSearchCircleSharp className="icon" onClick={() => setShowSearch(false)} />}
        {!userData && <FaCircleUser className="icon" onClick={() => setShowProfile(prev => !prev)} />}
        {userData && (
        ...
        <MdOutlineShoppingCart className="icon hidden md:block" onClick={() => navigate("/cart")} />
        <p className="cart-count hidden md:block">{getCartCount()}</p>
      </div>
      ...
      {showProfile && (
        <div className="profile-dropdown">
          <ul>
            {!userData && <li onClick={() => { navigate("/login"); setShowProfile(false); }}>Login</li>}
            {userData && <li onClick={() => { handleLogout(); setShowProfile(false); }}>Logout</li>}
      ...
      <div className="mobile-nav md:hidden">
        ...
        <button onClick={() => navigate("/collection")}><HiOutlineCollection /> Collections</button>
        <button onClick={() => navigate("/contact")}><MdContacts /> Contact</button>
        <button onClick={() => navigate("/cart")}><MdOutlineShoppingCart /> Cart</button>
        <p>{getCartCount()}</p>
      </div>
    </div>
  );
